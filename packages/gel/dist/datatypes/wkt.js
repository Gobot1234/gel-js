import { GeometryCollection, LineString, MultiLineString, MultiPoint, MultiPolygon, Point, Polygon, Triangle, TriangulatedIrregularNetwork, PolyhedralSurface, CircularString, CompoundCurve, CurvePolygon, MultiCurve, MultiSurface, } from "./postgis";
...
export function parseWKT(wkt) {
    ...
    function _parseGeom(unnamedGeom = null, allowedGeoms = null) {
        ...
        switch (type) {
            ...
            case "LINESTRING":
            case "CIRCULARSTRING":
                geom = _parseLineString(empty, type === "CIRCULARSTRING" ? CircularString : LineString);
                break;
            case "POLYGON":
            case "TRIANGLE":
                ...
                break;
            case "MULTIPOINT":
                geom = new MultiPoint(empty ? [] : _parseCommaSep(() => _parseBracketedGeom(_parsePoint)), hasZ ?? false, hasM ?? false, srid);
                break;
            case "MULTILINESTRING":
                geom = new MultiLineString(empty
                    ? []
            ...
            case "TIN":
                {
                    ...
                    geom = new Geom(empty
                        ? []
                        : _parseCommaSep(() => _parseBracketedGeom(() => _parsePolygon(false, type === "TIN" ? Triangle : Polygon))), hasZ ?? false, hasM ?? false, srid);
                }
                break;
            case "GEOMETRYCOLLECTION": {
                geom = new GeometryCollection(empty ? [] : _checkDimensions(() => _parseCommaSep(_parseGeom)), hasZ ?? false, hasM ?? false, srid);
                break;
            }
            case "COMPOUNDCURVE":
                {
                    ...
                    geom = new CompoundCurve(segments, hasZ ?? false, hasM ?? false, srid);
                }
                break;
            case "CURVEPOLYGON":
            case "MULTICURVE":
    ...
    function _parseLineString(empty = false, Geom = LineString) {
        return new Geom(empty ? [] : _parseCommaSep(_parsePoint), hasZ ?? false, hasM ?? false, srid);
    }
    function _parsePolygon(empty = false, Geom = Polygon) {
