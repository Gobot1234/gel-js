...
export function getSCRAM({ randomBytes, H, HMAC, makeKey }) {
    ...
    async function buildClientFinalMessage(password, salt, iterations, clientFirstBare, serverFirst, serverNonce) {
        ...
        const authMessage = utf8Encoder.encode(`${clientFirstBare},${serverFirst},${clientFinal}`);
        const saltedPassword = await _getSaltedPassword(utf8Encoder.encode(saslprep(password)), salt, iterations);
