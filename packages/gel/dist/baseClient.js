...
export class ClientConnectionHolder {
    ...
    async retryingFetch(query, args, outputFormat, expectedCardinality, language = Language.EDGEQL) {
        for (let iteration = 0;; ++iteration) {
    ...
    async execute(query, args) {
        await this.retryingFetch(query, args, OutputFormat.NONE, Cardinality.NO_RESULT);
    }
    async executeSQL(query, args) {
        await this.retryingFetch(query, args, OutputFormat.NONE, Cardinality.NO_RESULT, Language.SQL);
    }
    async query(query, args) {
        return this.retryingFetch(query, args, OutputFormat.BINARY, Cardinality.MANY);
    }
    async querySQL(query, args) {
        return this.retryingFetch(query, args, OutputFormat.BINARY, Cardinality.MANY, Language.SQL);
    }
    ...
    async queryRequired(query, args) {
        return this.retryingFetch(query, args, OutputFormat.BINARY, Cardinality.AT_LEAST_ONE);
    }
    async queryRequiredJSON(query, args) {
        return this.retryingFetch(query, args, OutputFormat.JSON, Cardinality.AT_LEAST_ONE);
    }
...
export class BaseClientPool {
    ...
    async getNewConnection() {
        ...
        const config = await this._getNormalizedConnectConfig();
        const connection = await retryingConnect(this._connectWithTimeout, config, this._codecsRegistry);
...
export class Client {
    ...
    async describe(query) {
        ...
        try {
            ...
            const result = await cxn._parse(Language.EDGEQL, query, OutputFormat.BINARY, Cardinality.MANY, this.options);
