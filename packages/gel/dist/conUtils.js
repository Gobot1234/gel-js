...
export function isValidTlsSecurityValue(candidate) {
    return (typeof candidate === "string" &&
        validTlsSecurityValues.includes(candidate));
}
export function getConnectArgumentsParser(utils) {
    return async (opts) => {
        return {
        ...
        return this._setParam("tlsSecurity", tlsSecurity, source, (_tlsSecurity) => {
            ...
            if (clientSecurity !== undefined) {
                if (!["default", "insecure_dev_mode", "strict"].includes(clientSecurity)) {
                    throw new InterfaceError(`invalid GEL_CLIENT_SECURITY value: '${clientSecurity}', ` +
    ...
    if (!hasCompoundOptions) {
        ...
        if (instName !== null) {
            ...
            if (database !== undefined && branch !== undefined) {
                if (database !== branch) {
                    throw new InterfaceError("Both database and branch exist in the config dir and don't match.");
                }
                else {
                    database = undefined;
                }
...
async function resolveConfigOptions(resolvedConfig, config, sources, compoundParamsError, serverUtils) {
    ...
    const readFile = serverUtils?.readFileUtf8 ??
        ((fn) => {
            throw new InterfaceError(`cannot read file "${fn}" in browser (or edge runtime) environment`);
        });
    ...
    if (config.database != null) {
        if (config.branch != null) {
            throw new InterfaceError(`${sources.database} and ${sources.branch} are mutually exclusive`);
        }
        ...
        if (config.dsn !== undefined ||
            ...
            if (config.credentials != null) {
                ...
                if (!serverUtils && !config.instanceName?.includes("/")) {
                    throw new InterfaceError(`cannot ${config.credentialsFile
                ...
                if (credentialsFile === undefined) {
                    if (/^\w(-?\w)*$/.test(config.instanceName)) {
                        credentialsFile = await getCredentialsPath(config.instanceName, serverUtils);
...
async function parseDSNIntoConfig(_dsnString, config, source, readFile) {
    ...
    if (searchParamsContainsBranch) {
        if (searchParamsContainsDatabase) {
            throw new InterfaceError(`invalid DSN: cannot specify both 'database' and 'branch'`);
        }
...
async function parseCloudInstanceNameIntoConfig(config, cloudInstanceName, source, serverUtils) {
    ...
    if (domainName.length > DOMAIN_NAME_MAX_LEN) {
        throw new InterfaceError(`invalid instance name: cloud instance name length cannot exceed ${DOMAIN_NAME_MAX_LEN - 1} characters: ${cloudInstanceName}`);
    }
    let secretKey = config.secretKey;
    if (secretKey == null) {
        try {
            if (!serverUtils) {
                throw new InterfaceError(`Cannot get secret key from cloud profile in browser (or edge runtime) environment`);
            }
            ...
            if (!secretKey) {
                throw new InterfaceError(`Cloud profile '${profile}' doesn't contain a secret key`);
            }
            config.setSecretKey(secretKey, `cloud-credentials/${profile}.json`);
        }
        catch (e) {
            throw new InterfaceError(`Cannot connect to cloud instances without a secret key: ${e}`);
        }
    }
    try {
        ...
        if (!dnsZone) {
            throw new InterfaceError("Invalid secret key: payload does not contain 'iss' value");
        }
...
function _jwtBase64Decode(payload) {
    return JSON.parse(utf8Decoder.decode(decodeB64(payload.padEnd(Math.ceil(payload.length / 4) * 4, "="))));
}
