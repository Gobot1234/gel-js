...
import { ProtocolError, InternalClientError, UnsupportedProtocolVersionError, } from "../errors";
...
export class CodecsRegistry {
    ...
    buildCodec(spec, protocolVersion) {
        if (!versionGreaterThanOrEqual(protocolVersion, [2, 0])) {
            throw new UnsupportedProtocolVersionError("unsupported old protocol version v1; downgrade to the previous " +
                "version of gel-js");
        }
        ...
        switch (t) {
            ...
            case CTYPE_SCALAR: {
                ...
                for (let i = 0; i < ancestorCount; i++) {
                    ...
                    if (!(ancestorCodec instanceof ScalarCodec)) {
                        throw new ProtocolError(`a scalar codec expected for base scalar type, ` +
                            `got ${ancestorCodec}`);
                    }
            ...
            case CTYPE_RECORD: {
                ...
                for (let i = 0; i < els; i++) {
                    ...
                    const subCodec = cl[pos];
                    if (subCodec == null) {
                        throw new ProtocolError("could not build record codec: missing subcodec");
                    }
                    codecs[i] = subCodec;
                }
