import { Box2D, Box3D, CircularString, CompoundCurve, CurvePolygon, Geometry, GeometryCollection, LineString, MultiCurve, MultiLineString, MultiPoint, MultiPolygon, MultiSurface, Point, Polygon, PolyhedralSurface, Triangle, TriangulatedIrregularNetwork, } from "../datatypes/postgis";
import { InternalClientError, InvalidArgumentError } from "../errors";
import { ScalarCodec } from "./ifaces";
export class PostgisGeometryCodec extends ScalarCodec {
    encode(buf, object, ctx) {
        if (ctx.hasOverload(this)) {
            ...
            if (!(object instanceof Geometry)) {
                throw new InvalidArgumentError(`a Geometry object was expected, got "${object}"`);
            }
...
export class PostgisBox2dCodec extends ScalarCodec {
    encode(buf, object, ctx) {
        let min;
        let max;
        if (ctx.hasOverload(this)) {
            ...
            if (!(object instanceof Box2D)) {
                throw new InvalidArgumentError(`a Box2D object was expected, got "${object}"`);
            }
            min = object.min;
            max = object.max;
        }
        const finalise = buf.writeDeferredSize();
        _encodeGeometry(buf, new Polygon([
            new LineString([
...
export class PostgisBox3dCodec extends ScalarCodec {
    encode(buf, object, ctx) {
        let min;
        let max;
        if (ctx.hasOverload(this)) {
            ...
            if (!(object instanceof Box3D)) {
                throw new InvalidArgumentError(`a Box3D object was expected, got "${object}"`);
            }
            min = object.min;
            max = object.max;
        }
    ...
    decode(buf, ctx) {
        ...
        else if (poly.constructor === PolyhedralSurface &&
...
function _parseGeometry(buf, srid = null) {
    ...
    switch (type) {
        ...
        case 16:
            return _parseMultiPolygon(buf, TriangulatedIrregularNetwork, le, z, m, srid);
        case 17:
...
function _parsePoint(buf, le, z, m, srid) {
    return new Point(buf.readFloat64(le), buf.readFloat64(le), z ? buf.readFloat64(le) : null, m ? buf.readFloat64(le) : null, srid);
}
    ...
    for (let i = 0; i < surfaceCount; i++) {
        ...
        switch (type) {
            case 3:
                ...
                break;
            case 10:
                surfaces[i] = _parseMultiCurve(buf, CurvePolygon, le, z, m, srid);
                break;
            default:
